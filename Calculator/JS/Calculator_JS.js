...
function Input_Digit(digit) {
    ...
    if (Wait_Second_Operand === true)   {
        ...
        // This overwites Display_Value if the current value is 0, otherwise it adds onto it.
...
function Input_Decimal(dot) {
    // This ensures that accidental clicking of the decimal point doesnt cause bugs in your operation.
...
function Handle_Operator(Next_Operator) {
    ...
    // When an operator key is pressed, we convert the current number displayed in the screen to a number and then store the result in Calculator.First_Operand if it doesn't already exist.
    ...
    // This information updates the screen with the contents of Display_Value
    ...
    keys.addEventListener('click', (event) => {
        ...
        const { target } = event;
        // If the element that was clicked on is not a button, exit the funciton.
